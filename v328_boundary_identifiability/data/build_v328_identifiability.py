"""Build the V328 boundary-identifiability analysis and publication figures."""
...
def build_typed_audit(audit: pd.DataFrame) -> pd.DataFrame:
    ...
    typed.to_csv(DATA / "typed_audit_16_v328.csv", index=False, encoding="utf-8-sig")
    return typed
...
def build_comparability_graph(typed: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    write_csv(DATA / "comparability_edges_v328.csv", edges)
    ...
    (DATA / "comparability_graph_metrics_v328.json").write_text(
...
def build_repair_analysis(audit: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, int]:
    ...
    coalition_df.to_csv(DATA / "repair_coalitions_v328.csv", index=False, encoding="utf-8-sig")
    ...
    shapley_df.to_csv(DATA / "repair_shapley_v328.csv", index=False, encoding="utf-8-sig")
...
def build_rule_and_deletion_sensitivity(audit: pd.DataFrame) -> dict[str, object]:
    ...
    rule_df.to_csv(DATA / "coding_rule_sensitivity_v328.csv", index=False, encoding="utf-8-sig")
    shapley_df = pd.DataFrame(shapley_rows)
    shapley_df.to_csv(
        DATA / "repair_shapley_rule_sensitivity_v328.csv", index=False, encoding="utf-8-sig"
    )
    deletion_df = pd.DataFrame(deletion_rows)
    deletion_df.to_csv(
        DATA / "leave_one_record_out_shapley_v328.csv", index=False, encoding="utf-8-sig"
    )
    ...
    pd.DataFrame(loo_summary).to_csv(
        DATA / "leave_one_record_out_summary_v328.csv", index=False, encoding="utf-8-sig"
    )
    ...
    (DATA / "boundary_clustering_null_diagnostic_v328.json").write_text(
    ...
    (DATA / "rule_and_deletion_sensitivity_summary_v328.json").write_text(
...
def build_numeric_cases() -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    cases.to_csv(DATA / "numeric_cases_9_v328.csv", index=False, encoding="utf-8-sig")
    ...
    (DATA / "numeric_case_summary_v328.json").write_text(
        json.dumps(summary, indent=2), encoding="utf-8"
    )
...
def build_partial_identification_rules() -> None:
    ...
    write_csv(DATA / "partial_identification_rules_v328.csv", rows)
...
def figure_2_graph(typed: pd.DataFrame, edges: pd.DataFrame, metrics: dict[str, object]) -> None:
    fig, ax = plt.subplots(figsize=(7.2, 4.1))
    positions: dict[str, tuple[float, float]] = {}
    for x, route in enumerate(ROUTE_ORDER):
        subset = typed[typed.route_family == route].sort_values("record_id")
        ys = np.linspace(0.8, 5.2, len(subset)) if len(subset) > 1 else [3.0]
        for y, record_id in zip(ys, subset.record_id):
            positions[record_id] = (x, float(y))
        ax.text(x, 0.15, route.replace(" ", "\n"), ha="center", va="top", fontsize=6.7)

    for _, edge in edges.iterrows():
        x1, y1 = positions[edge.record_i]
        x2, y2 = positions[edge.record_j]
        ax.plot([x1, x2], [y1, y2], color="#1F6F78", linewidth=2.4, zorder=1)
    ...
    for _, row in typed.iterrows():
        ...
        ax.scatter(
            x,
            y,
            s=130 if closed else 85,
            color=ROUTE_COLORS[row.route_family] if closed else "#F4F4F4",
            edgecolor=ROUTE_COLORS[row.route_family],
            linewidth=1.4 if closed else 1.0,
            zorder=2,
        )
        ax.text(x, y, row.record_id, ha="center", va="center", fontsize=6.4, fontweight="bold" if closed else "normal")

    ax.set_xlim(-0.6, len(ROUTE_ORDER) - 0.4)
    ax.set_ylim(-0.05, 6.25)
    ax.axis("off")
    ax.text(
        0.0,
        5.95,
        f"1 identified edge / {metrics['possible_pairs']} possible pairs | density {metrics['graph_density']:.4f} | 14 isolates",
        ha="left",
        fontsize=8.1,
        fontweight="bold",
    )
    ax.text(
        0.0,
        5.55,
        "Edge criterion: common nominal-magnetron source plane, complete exposure, accepted formed mass and formation-yield endpoint.",
        ha="left",
        fontsize=7.0,
        color="#444444",
    )
    ax.set_title("The declared target-metric comparability graph is fragmented", loc="left", fontweight="bold")
...
def figure_3_repair(coalitions: pd.DataFrame, shapley: pd.DataFrame, interaction: int) -> None:
    ...
    for _, row in coalitions.iterrows():
        if row.repair_fields in {"none", "mass", "mass;energy", "mass;energy;time", "mass;energy;time;endpoint"}:
            ...
            ax.annotate(label, (row.field_count, row.formation_closed_records_upper_bound), xytext=(4, 5), textcoords="offset points", fontsize=6.7)
    ax.set_xticks(range(5))
    ax.set_xlabel("Universally supplied coordinate fields")
    ax.set_ylabel("Formation-closed records (upper bound)")
    ax.set_ylim(0, 17)
    ax.grid(axis="y", color="#DDDDDD", linewidth=0.6)
    ax.set_title("a  Closure requires complementary repairs", loc="left", fontweight="bold")
    ...
    ax.invert_yaxis()
    ax.set_xlabel("Shapley closure gain (records)")
    ax.set_xlim(0, 7)
    ...
    ax.set_title(f"b  Shapley gains; mass-energy interaction = +{interaction}", loc="left", fontweight="bold")
    fig.suptitle("A one-field reporting fix is insufficient because missing coordinates interact", x=0.01, ha="left", fontweight="bold", fontsize=10.5)
...
def figure_4_denominator(cases: pd.DataFrame) -> None:
    ...
    ax.set_title("a  Yield changes the denominator", loc="left", fontweight="bold")
    ...
    fig.suptitle("Identical feed-normalized input can imply different product-normalized burdens", x=0.01, ha="left", fontweight="bold", fontsize=10.5)
    fig.tight_layout()
...
def figure_5_rank_ambiguity(summary: dict[str, object]) -> None:
    ...
    ax.axvspan(1 / cmax, cmax, color="#F2C14E", alpha=0.22, label="rank not fixed by nominal ratio")
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlim(0.2, 5)
    ax.set_ylim(0.08, 12)
    ...
    ax.set_xlabel("Nominal feed-normalized intensity ratio, route i / route j")
    ax.set_ylabel("Admissible formed-product intensity ratio")
    ...
    ax.set_title("Yield variation alone leaves nominal ratios from 0.44 to 2.27 rank-ambiguous", loc="left", fontweight="bold")
...
def write_reports(
    ...
    report = f"""# V328 computation verification
...
- Mass-energy discrete interaction: +{interaction} optimistic closures
...
## Figure 2
Sixteen study nodes are arranged by eight process families. Only B08 and B09,
both microwave records, are connected. Fourteen nodes are isolated for the
declared nominal-source-input per formed-mass comparison.
