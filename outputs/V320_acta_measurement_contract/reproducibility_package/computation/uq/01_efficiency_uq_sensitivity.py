...
baseline, or robust to the assumed parameters?  (Answers reviewer-sim M1.)
...
guarantees it reproduces script 02's declared-baseline 6.0% / 1.5% before any uncertainty is propagated):
...
def main():
    ...
    # (0) baseline reproduces script 02 -> guard against model drift
    eff_b, Td_b = eff_curve(**BASE, dwells=DWELLS)
    e900, e3600 = eff_b[I900] * 100, eff_b[I3600] * 100
    print(f"baseline (script-02 declared geometry): eff@900s={e900:.2f}%  eff@3600s={e3600:.2f}%  "
    ...
    print(f"  band brackets the declared-baseline points? @900 {lo[I900]:.1f}<= {e900:.1f} <={hi[I900]:.1f} ; "
    ...
    print(f"  (independent of coupling multiplier; confirms re-radiation dominates the ledger)\n")
