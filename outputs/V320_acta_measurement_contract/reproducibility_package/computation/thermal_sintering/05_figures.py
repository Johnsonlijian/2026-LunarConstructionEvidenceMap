...
def savefig(fig, stem, formats=("png", "pdf"), dpi=600):
    ...
    for fmt in formats:
        fig.savefig(stem.with_suffix("." + fmt), bbox_inches="tight", dpi=dpi)
...
def fig_mechanism():
    ...
    try:
        ...
        ax2.collections[-1].set_label("5-95% under parameter uncertainty")
    except FileNotFoundError:
        pass
    ax2.plot(d, eff, "-", color="#444", lw=1.6, zorder=2, label="ledger model")
    for name, P, dwell, rep, c in cases:
...
def fig_closure():
    ...
    energy = d["energy_grid_MJkg"]
    boundary = d["boundary_kW"]
    YMAX = 12.0
    fig, ax = plt.subplots(figsize=(WIDTH_SINGLE*1.9, WIDTH_SINGLE*1.5))
    ax.fill_between(energy, boundary, YMAX, color="#e3f1e7", alpha=0.75, zorder=0)
    ax.fill_between(energy, 0, boundary, color="#f7ece4", alpha=0.55, zorder=0)
    ax.plot(energy, boundary, color="#444", lw=1.8, label="wall-input capacity boundary")
    e_lo, e_hi = float(d["E_lo_MJkg"]), float(d["E_hi_MJkg"])
    c_lo, c_hi = float(d["cap_lo"]), float(d["cap_hi"])
    ax.scatter([e_lo, e_hi], [c_lo, c_hi], color=["#2f8f4e", "#b5400a"], s=46, zorder=5,
               edgecolor="white", linewidth=0.8)
    ax.plot([e_lo, e_hi], [c_lo, c_hi], color="#777", lw=1.0, ls=(0, (4, 3)))
    ax.annotate(f"Lim 2021\n18 MJ/kg -> {c_lo:.1f} kW", (e_lo, c_lo), xytext=(24, c_lo + 1.4),
                fontsize=6.5, color="#2f8f4e", ha="left",
                arrowprops=dict(arrowstyle="->", color="#2f8f4e", lw=0.8))
    ax.annotate(f"Lim 2023\n72 MJ/kg -> {c_hi:.1f} kW", (e_hi, c_hi), xytext=(51, c_hi + 1.2),
                fontsize=6.5, color="#b5400a", ha="left",
                arrowprops=dict(arrowstyle="->", color="#b5400a", lw=0.8))
    ax.text(14, 10.5, "CLOSES\nif deployed wall-plug capacity\nlies above the boundary",
            fontsize=6.6, color="#1f6e3a", ha="left", va="top")
    ax.text(62, 2.0, "does not close", fontsize=7.5, color="#a23b2b", fontweight="bold")
    ax.annotate(f"4x spread:\n{c_lo:.1f}-{c_hi:.1f} kW",
                ((e_lo + e_hi) / 2, (c_lo + c_hi) / 2), xytext=(32, 7.4),
                fontsize=6.4, color="#333", ha="left",
                arrowprops=dict(arrowstyle="->", color="#555", lw=0.8))
    ax.text(49, 11.25, f"diagnostic mission: {int(d['M_req'])} kg in {int(d['window_days'])} d; "
            "energy/mass is reported wall-plug input", fontsize=6.2, color="#666",
            ha="center", style="italic", bbox=dict(boxstyle="round,pad=0.3", fc="#fbf1df", ec="#e0871a", lw=0.8))
    ax.set_xlim(energy[0], energy[-1]); ax.set_ylim(0, YMAX)
    ax.set_xlabel("wall-plug input energy per accepted mass (MJ kg$^{-1}$)", fontsize=8.0)
    ax.set_ylabel("deployed wall-plug source capacity (kW)  (unreported, 0/7)", fontsize=8.0)
    ax.set_title("Physics-derived capacity boundary without double-counting efficiency\n"
                  "(the two reported wall-energy values imply a fourfold capacity spread)",
                  fontsize=8.2, fontweight="bold")
    ax.legend(fontsize=6.4, loc="lower right", frameon=True, framealpha=0.9)
    for s in ("top", "right"):
