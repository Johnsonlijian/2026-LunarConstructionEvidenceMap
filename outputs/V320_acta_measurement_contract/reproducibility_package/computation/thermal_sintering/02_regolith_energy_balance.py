#!/usr/bin/env python3
"""Regolith directed-energy sintering: declared-geometry transient energy ledger.
...
Physical point (the mechanism the manuscript currently only asserts):
  total wall-plug input energy per accepted mass is reported as P_wall * dwell / mass.
...
unreported. The script uses the reported Lim wall-input energy values as ledger inputs; it does not
fit them by dividing through a coupling efficiency.
...
def simulate(mass_kg, P_input_W, eta_couple, dwell_s, t_ramp_s=None,
    ...
    q_rad = EPS * SIGMA * A * (T**4 - T_AMB**4)
    E_rad = np.trapezoid(q_rad[on], t[on])
...
def main():
    print("== Component 2: regolith sintering energy ledger, declared geometry ==\n")
    ...
    # --- two reported wall-input energy values from the census ---
    ...
    print("Same reported wall-input power, different dwell -> two energy-per-mass ledger values:")
    ...
    for name, P, dwell, rep in cases:
        # declared coupling that reaches sintering for the short-dwell case; keep it fixed across both
        eta = 0.60
    ...
    print("\nsaved out_02_calibration.npz (legacy filename; dwell sweep + temperature history for figures)")
