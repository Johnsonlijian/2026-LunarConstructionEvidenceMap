...
census (Figure 2 / Section 5), (2) the worked-example directed-energy closure margins
for completions A and B and the closure-boundary thresholds (Figure 1b), and (3) the
wall-input capacity boundary from the two reported Lim energy-per-mass values (Figure P3).
"""
import csv
...
def reproduce_worked_example() -> None:
    ...
    e_star = 30.0                                 # MJ/kg (declared illustrative energy budget)
    M_req, T_build_days, utilization = 2400.0, 100.0, 1.0
    T_build_s = T_build_days * 24 * 3600.0
    # Required simultaneously active modules from the reported specimen throughput.
    N_req = -(-M_req * t_s // (m_s * T_build_s * utilization))
    ...
    print(f"  required active modules from throughput: N_req = {N_req:.0f}")
...
def reproduce_wall_energy_boundary() -> None:
    M_req, window_days = 1000.0, 100.0
    window_s = window_days * 24 * 3600.0
    print("\n== Physics-derived wall-input capacity boundary ==")
    for e_wall in (18.0, 72.0):
        p_cap = M_req * e_wall * 1e6 / window_s / 1000.0
        print(f"  E_wall = {e_wall:4.1f} MJ/kg -> P_cap = {p_cap:.2f} kW")
    print("  -> no wall-plug-to-cavity efficiency divisor is applied to already wall-input energy.")


if __name__ == "__main__":
    reproduce_census()
    reproduce_worked_example()
    reproduce_wall_energy_boundary()
